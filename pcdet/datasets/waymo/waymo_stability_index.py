...
def eval_waymo_stability_index(cur_det_annos, pre_det_annos, cur_gt_annos, pre_gt_annos, class_names):
...
def print_stability_index_results(metrics, class_names):
    ...
    metrics_data_print = []                                                    
    metrics_str += '\n\n----------------------------------\n'
    metrics_str += f'Stability Index (BreakDown By Distance)'
    metrics_str += '\n----------------------------------\n'
    ...
    for class_name in class_names:
        for idx, distance in enumerate([[0, 30], [30, 50], [50, np.float('inf')]]):
            ...
            metrics_data_print.append([class_name if idx == 0 else '', 
                            ['<30', '30-50', '>50'][idx],
                            f"{metrics['STABILITY_INDEX_%s_%s_to_%s' % (class_name, MIN, MAX)]:.4f}",
